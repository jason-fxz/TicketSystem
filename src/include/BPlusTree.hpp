...
#define MIN_LEAF_SIZE ((L + 1) / 2)
  ...
  public:
    ...
    void search(const Key_t &key_L, const Key_t &key_R, vector<Data_t> &res) {
        if (m_size == 0) return;
        BNodePtr cur = get_node(m_root);
        while (cur->is_inner()) {
            int i = 0;
    ...
    pair<Data_t, bool> find(const Key_t &key) {
        vector<Data_t> tmp;
        search(key, key, tmp);
        return tmp.size() > 0 ? pair(tmp[0], true) : pair(Data_t(), false);
    }


    void modify(const Key_t &key, const Data_t &data) {
        if (m_size == 0) return;
        BNodePtr cur = get_node(m_root);
        while (cur->is_inner()) {
            int i = 0;
            while (i < cur->count - 1
                   && Camp(cur.as_inner()->key[i], key) <= 0) ++i;
            cur = get_node(cur.as_inner()->child[i]);
        }
        BNodePtr leaf = cur;
        while (true) {
            if (Camp(leaf.as_leaf()->key[leaf->count - 1], key) < 0) {
                if (leaf.as_leaf()->next == 0) break;
                leaf = get_node(leaf.as_leaf()->next);
                continue;
            }
            int i = 0;
            while (i < leaf->count && Camp(leaf.as_leaf()->key[i], key) < 0) ++i;
            if (i < leaf->count && Camp(leaf.as_leaf()->key[i], key) == 0) {
                leaf.as_leaf()->data[i] = data;
                leaf.set_dirty();
                return ;
            }
            if (i == leaf->count && leaf.as_leaf()->next != 0) {
                leaf = get_node(leaf.as_leaf()->next);
            } else {
                break;
            }
...
template < typename Key, typename Tp,
           ...
           const bool enable_file_recycle = true
           >
class BPlusTreeBigData {
    BPlusTree<Key, int, FILE_BLOCK_SIZE, MAX_CACHE_SIZE, enable_file_recycle> BPT;
    DataFile<Tp> data_file;
public:
    BPlusTreeBigData(std::string data_file_name) : BPT(data_file_name), data_file(data_file_name) {
    }

    ~BPlusTreeBigData() {
    }

    void insert(const Key &key, const Tp &data) {
        int index = data_file.write(data);
    ...
    pair<Tp, bool> find(const Key &key) {
        auto res = BPT.find(key);
        Tp tmp;
        data_file.read(tmp, res.first);
        if (res.second) {
            return pair(tmp, true);
        } else {
            return pair(Tp(), false);
        }
    }

    void modify(const Key &key, const Tp &data) {
        auto res = BPT.find(key);
        if (res.second) {
            data_file.update(data, res.first);
        } else throw "Key not found";
    }

    void search(const Key &key_L, const Key &key_R, vector<Tp> &res) {
        vector<int> index;
        BPT.search(key_L, key_R, index);
        for (auto &x : index) {
            res.push_back(data_file.read(x));
        }
    }
